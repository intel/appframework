...
a)};c.prototype={startX:0,startY:0,dx:0,dy:0,myDivWidth:0,myDivHeight:0,cssMoveStart:0,childrenCount:0,carouselIndex:0,vertical:!1,horizontal:!0,el:null,movingElement:!1,container:null,pagingDiv:null,pagingCssName:"carousel_paging",pagingCssNameSelected:"carousel_paging_selected",pagingCallback:null,lockMove:!1,touchStart:function(a){this.myDivWidth=numOnly(this.container.clientWidth);this.myDivHeight=numOnly(this.container.clientHeight);this.lockMove=!1;if(event.touches[0].target&&event.touches[0].target.type!==
...
b=g=null;this.pagingDiv.style.width=this.childrenCount*50+"px";this.pagingDiv.style.height="25px"}this.onMoveIndex(this.carouselIndex)}};return c}}()})(jq);(function(i){i.fn.css3Animate=function(c){for(var b,d=0;d<this.length;d++)b=new e(this[d],c);return this.length==1?b:this};i.css3AnimateQueue=function(){return new e.queue};var e=function(){if(window.WebKitCSSMatrix){var c="m11"in new WebKitCSSMatrix?"3d(":"(",b="m11"in new WebKitCSSMatrix?",0)":")",d=function(h,a){this.el=typeof h=="string"||h instanceof String?document.getElementById(h):h;if(!(this instanceof d))return new d(h,a);if(this.el){var f=this;if(a){this.el.addEventListener("webkitTransitionEnd",
f.finishAnimation,!1);a.callback?(this.callback=a.callback,this.moving=!0,this.timeout=window.setTimeout(function(){if(f.moving==!0&&f.callback&&typeof(f.callback=="function"))f.moving=!1,f.callback(),delete this.callback},numOnly(a.time)+50)):this.moving=!1;a.y||(a.y=0);a.x||(a.x=0);a.previous&&(a.y+=numOnly((new WebKitCSSMatrix(window.getComputedStyle(this.el).webkitTransform)).f),a.x+=numOnly((new WebKitCSSMatrix(window.getComputedStyle(this.el).webkitTransform)).e));if(!a.origin)a.origin="0% 0%";
...
"px";this.el.style.webkitTransform="translate"+c+a.x+","+a.y+b+" scale("+parseFloat(a.scale)+") rotate("+a.rotateX+") rotateY("+a.rotateY+") skew("+a.skewX+","+a.skewY+")";this.el.style.webkitBackfaceVisiblity="hidden";this.el.style.webkitTransition="all "+a.time+" "+a.timingFunction;this.el.style.webkitTransformOrigin=a.origin;if(a.width)this.el.style.width=a.width;if(a.height)this.el.style.height=a.height;if(a.opacity!==void 0)this.el.style.opacity=a.opacity}else alert("Please provide configuration options for animation of "+
h)}};d.prototype={finishAnimation:function(b){b.preventDefault();if(this.moving)this.moving=!1,this.el.removeEventListener("webkitTransitionEnd",this.finishAnimation,!0),this.callback&&typeof(this.callback=="function")&&(this.timeout&&window.clearTimeout(this.timeout),this.callback(),delete this.callback)}};return d}}();e.queue=function(){return{elements:[],push:function(c){this.elements.push(c)},pop:function(){return this.elements.pop()},run:function(){var c=this;if(this.elements.length!=0&&(typeof this.elements[0]==
...
this.vscrollBar.webkitTransition="",this.hscrollBar.style.opacity=1}}catch(o){alert("error in scrollStart: "+o)}}},touchMove:function(a){try{if(this.currentScrollingObject!=null){a.preventDefault();var b={x:0,y:0},d=0,c=0,e=0,h=0;if(this.verticalScroll&&this.maxTop>0){var i=this.lockY-a.touches[0].pageY,i=-i,d=this.startTop+i;try{c=numOnly((new WebKitCSSMatrix(window.getComputedStyle(this.el).webkitTransform)).f)}catch(m){c=0}b.y=d}if(this.horizontalScroll&&this.maxLeft>0){var o=this.lockX-a.touches[0].pageX,
o=-o,i=e=this.startLeft+o;try{h=-numOnly((new WebKitCSSMatrix(window.getComputedStyle(this.el).webkitTransform)).e)}catch(y){h=0}b.x=i}if(this.maxTop>0&&(b.y>0||b.y<-1*this.maxTop)){var r=(this.container.clientHeight-(b.y>0?b.y:-1*(b.y+this.maxTop)))/this.container.clientHeight;r<0.5&&(r=0.5);b.y>0?b.y*=r:b.y-=(b.y+this.maxTop)*r}this.scrollerMoveCSS(this.currentScrollingObject,b,0);if(this.vscrollBar){var s=this.bottomMargin-numOnly(this.vscrollBar.style.height)-(this.maxTop+d)/this.maxTop*(this.bottomMargin-
numOnly(this.vscrollBar.style.height));this.scrollerMoveCSS(this.vscrollBar,{x:0,y:s},0)}this.hscrollBar&&(s=this.rightMargin-numOnly(this.hscrollBar.style.width)-(this.maxLeft+e)/this.maxLeft*(this.rightMargin-numOnly(this.hscrollBar.style.width)),this.scrollerMoveCSS(this.hscrollBar,{x:s,y:0},0));if(this.prevTime){var v=Math.abs(c)-Math.abs(d),w=Math.abs(h)-Math.abs(e),t=a.timeStamp-this.prevTime;t<500?(this.timeMoved+=t,this.vdistanceMoved+=v,this.hdistanceMoved+=w):this.hdistanceMoved=this.vdistanceMoved=
this.timeMoved=0}this.prevTime=a.timeStamp}}catch(x){alert("error in scrollMove: "+x)}},touchEnd:function(a){if(this.currentScrollingObject!=null){a.preventDefault();this.finishScrollingObject=this.currentScrollingObject;this.currentScrollingObject=null;var a={x:0,y:0},b=300,d;if(this.verticalScroll&&this.maxTop>0){var c=-this.vdistanceMoved,b=this.timeMoved,e=numOnly((new WebKitCSSMatrix(window.getComputedStyle(this.el).webkitTransform)).f);d=e;c=this.calculateMomentum(c,b);b=c.time;e<0&&(e-=c.dist);
e>0&&(e=0,b=200);e<-this.maxTop&&(e=-this.maxTop,b=200);a.y=e}if(this.horizontalScroll&&this.maxLeft>0)c=-this.hdistanceMoved,b=this.timeMoved,e=(new WebKitCSSMatrix(window.getComputedStyle(this.el).webkitTransform)).e,c=this.calculateMomentum(c,b),b=c.time,e<0&&(e-=c.dist),e>0&&(e=0,b=200),e<-this.maxLeft&&(e=-this.maxLeft,b=200),a.x=e;var h=this;this.refresh&&d>60&&this.refreshFunction&&this.refreshFunction.call();b<300&&(b=300);this.scrollerMoveCSS(this.finishScrollingObject,a,b,"cubic-bezier(0.33,0.66,0.66,1)");
if(this.vscrollBar){d=this.bottomMargin-numOnly(this.vscrollBar.style.height)-(this.maxTop+a.y)/this.maxTop*(this.bottomMargin-numOnly(this.vscrollBar.style.height));if(d>this.bottomMargin)d=this.bottomMargin;d<0&&(d=0);this.scrollerMoveCSS(this.vscrollBar,{x:0,y:d},b,"cubic-bezier(0.33,0.66,0.66,1)")}if(this.hscrollBar){d=this.rightMargin-numOnly(this.hscrollBar.style.width)-(this.maxLeft+a.x)/this.maxLeft*(this.rightMargin-numOnly(this.hscrollBar.style.width));if(d>this.rightMargin)d=this.rightMargin;
d<0&&(d=0);this.scrollerMoveCSS(this.hscrollBar,{x:d,y:0},b,"cubic-bezier(0.33,0.66,0.66,1)")}isNaN(b)?(h.hideScrollbars(),h.elementScrolling=!1):this.scrollingFinishCB=setTimeout(function(){h.hideScrollbars();h.elementScrolling=!1},b);this.elementScrolling=!0}this.vdistanceMoved=this.hdistanceMoved=0},scrollerMoveCSS:function(a,b,e,i){e||(e=0);i||(i="linear");a.style.webkitTransform="translate"+d+b.x+"px,"+b.y+"px"+c;a.style.webkitTransitionDuration=e+"ms";a.style.webkitBackfaceVisiblity="hidden";
a.style.webkitTransitionTimingFunction=i},scrollTo:function(a,b){b||(b=0);this.scrollerMoveCSS(this.el,a,b);if(this.vscrollBar){a=this.bottomMargin-numOnly(this.vscrollBar.style.height)-(this.maxTop+a.y)/this.maxTop*(this.bottomMargin-numOnly(this.vscrollBar.style.height));if(a>this.bottomMargin)a=this.bottomMargin;a<0&&(a=0);this.scrollerMoveCSS(this.vscrollBar,{x:0,y:a},b,"ease-out");this.vscrollBar.style.opacity="0"}if(this.hscrollBar){a=this.rightMargin-numOnly(this.hscrollBar.style.width)-(this.maxLeft+
...
c.style.backgroundColor="white";c.className="jqmobiSelect_fakeInput "+d.className;c.id=d.id+"_jqmobiSelect";c.style.border="1px solid gray";c.style.color="black";c.linkId=d.id;c.onclick=function(a){a.preventDefault();b.initDropDown(this.linkId)};d.parentNode.appendChild(c);d.parentNode.style.position="relative";d.style.display="none";d.style.webkitAppearance="none";for(a=0;a<d.options.length;a++){if(d.options[a].selected)c.value=d.options[a].text;d.options[a].watch("selected",function(a,d,c){if(c==
!0)b.updateMaskValue(this.parentNode.id,this.text,this.value),this.parentNode.value=this.value;return c})}d.watch("selectedIndex",function(a,d,c){if(this.options[c])b.updateMaskValue(this.id,this.options[c].text,this.options[c].value),this.value=this.options[c].value;return c});imageMask=c=null;b.oldSelects[d.id]=1}(e[c]);b.createHtml()}else alert("Could not find container element for jq.web.selectBox "+e)}else alert("This library requires jq.web.Scroller")},updateDropdown:function(e){if(e=document.getElementById(e)){for(var c=
0;c<e.options.length;c++){if(e.options[c].selected)fakeInput.value=e.options[c].text;e.options[c].watch("selected",function(b,d,c){if(c==!0)that.updateMaskValue(this.parentNode.id,this.text,this.value),this.parentNode.value=this.value;return c})}e=null}},initDropDown:function(e){var c=this,b=document.getElementById(e);if(!b.disabled&&b&&b.options&&b.options.length!=0){var d=0;document.getElementById("jqmobiSelectBoxScroll").innerHTML="";document.getElementById("jqmobiSelectBoxHeaderTitle").innerHTML=
b.name!=void 0&&b.name!="undefined"&&b.name!=""?b.name:e;for(var h=0;h<b.options.length;h++){b.options[h].watch("selected",function(a,b,d){if(d==!0)c.updateMaskValue(this.parentNode.id,this.text,this.value),this.parentNode.value=this.value;return d});var a=b.value==b.options[h].value?!0:!1,f="",g=document.createElement("div");g.className="jqmobiSelectRow";a?(f="checked",d=h,a="id='jqmobiSelectBox_found'",g.className="jqmobiSelectRowFound"):a="";g.id=a;g.style.cssText=";line-height:40px;font-size:14px;padding-left:10px;height:40px;width:100%;position:relative;width:100%;border-bottom:1px solid black;background:white;";
var i=document.createElement("a");i.href="javascript:;";g.tmpValue=h;g.onclick=function(){c.setDropDownValue(e,this.tmpValue)};i.style.cssText="text-decoration:none;color:black;";i.innerHTML=b.options[h].text;var p=document.createElement("span");p.style.cssText="float:right;margin-right:20px;margin-top:-2px";var l=document.createElement("button");a?(l.style.cssText="background: #000;padding: 0px 0px;border-radius:15px;border:3px solid black;",l.className="jqmobiSelectRowButtonFound"):(l.style.cssText=
"background: #ffffff;padding: 0px 0px;border-radius:15px;border:3px solid black;",l.className="jqmobiSelectRowButton");l.style.width="20px";l.style.height="20px";l.checked=f;i.className="jqmobiSelectRowText";p.appendChild(l);g.appendChild(i);g.appendChild(p);document.getElementById("jqmobiSelectBoxScroll").appendChild(g);i=l=p=null}try{document.getElementById("jqmobiSelectModal").style.display="block"}catch(u){console.log("Error showing div "+u)}try{if(g){var q=numOnly(g.style.height),m=numOnly(document.getElementById("jqmobiSelectBoxHeader").style.height);
this.scroller.scrollTo({x:0,y:d*q+m>=numOnly(document.getElementById("jqmobiSelectBoxFix").clientHeight)-m?d*-q+m:0})}}catch(o){console.log("error init dropdown"+o)}b=g=null}},updateMaskValue:function(e,c,b){var d=document.getElementById(e+"_jqmobiSelect"),e=document.getElementById(e);if(d)d.innerHTML=c;if(typeof e.onchange=="function")e.onchange(b)},setDropDownValue:function(e,c){var b=document.getElementById(e);if(b)b.selectedIndex=c;this.scroller.scrollTo({x:0,y:0});this.hideDropDown()},hideDropDown:function(){document.getElementById("jqmobiSelectModal").style.display=
