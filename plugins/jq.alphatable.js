		...
		alphaTable.prototype = {
			...
			scrollToLetter:function (letter){
				...
				if(el){
					...
					this.scroller.scrollTo({x:0,y:yPos});
				}
			},
			setupIndex:function(){
				...
				containerDiv.addEventListener("touchend",function(event){that.isMoving=false;that.clearLetterBox()},false);
				...
				this.scroller.scrollTo({x:0,y:0});//There's a bug with webkit and css3.  The letterbox would not show until -webkit-transform as applied.
				containerDiv=null;
	...
	(function(){
		...
		for(var j=0;j<document.styleSheets.length;j++){
			...
			for (var i = 0; i < rules.length; i++) {
				...
				if(name==undefined||(!name.length||name.length==0))
				   continue;
