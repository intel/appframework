if(!window.jq||typeof jq!=="function"){var jq=function(g){function u(a){return a in s?s[a]:s[a]=RegExp("(^|\\s)"+a+"(\\s|$)")}function o(a){for(var c=0;c<a.length;c++)a.indexOf(a[c])!=c&&a.splice(c,1);return a}function v(a,c){var b=[];if(a==f)return b;for(;a;a=a.nextSibling)a.nodeType==1&&a!==c&&b.push(a);return b}function n(){}function w(a,c){a.os={};a.os.webkit=c.match(/WebKit\/([\d.]+)/)?!0:!1;a.os.android=c.match(/(Android)\s+([\d.]+)/)||c.match(/Silk-Accelerated/)?!0:!1;a.os.ipad=c.match(/(iPad).*OS\s([\d_]+)/)?
...
a.length;b++){if(c(b,a[b])===!1)break}else if(d.isObject(a))for(b in a)if(a.hasOwnProperty(b)&&c(b,a[b])===!1)break;return a};d.extend=function(a){a==f&&(a=this);if(arguments.length===1){for(var c in a)this[c]=a[c];return this}else F.call(arguments,1).forEach(function(b){for(var c in b)a[c]=b[c]});return a};d.isArray=function(a){return a!=f&&a instanceof Array&&a.push!=f};d.isFunction=function(a){return a!=f&&typeof a==="function"};d.isObject=function(a){return a!=f&&typeof a==="object"};d.fn=m.prototype=
{constructor:m,forEach:l.forEach,reduce:l.reduce,push:l.push,indexOf:l.indexOf,concat:l.concat,selector:function(a,c){var b;try{if(a=a.trim(),a[0]==="#"&&a.indexOf(" ")===-1&&a.indexOf(">")===-1)b=c==j?c.getElementById(a.replace("#","")):[].slice.call(c.querySelectorAll(a));else if(a[0]==="<"&&a[a.length-1]===">"){var e=j.createElement("div");e.innerHTML=a.trim();b=[].slice.call(e.childNodes)}else b=[].slice.call(c.querySelectorAll(a))}catch(d){}return b},oldElement:f,slice:l.slice,setupOld:function(a){if(a==
...
c;for(var b=0;b<this.length;b++)this[b].dispatchEvent(a);return this};d.Event=function(a,c){var b=j.createEvent(I[a]||"Events"),e=!0;if(c)for(var d in c)d=="bubbles"?e=!!c[d]:b[d]=c[d];b.initEvent(a,e,!0,null,null,null,null,null,null,null,null,null,null,null,null);return b};d.proxy=function(a,c){return function(b){return a.call(c,b)}};return d}(window);"$"in window||(window.$=jq);if(!window.numOnly)var numOnly=function(g){isNaN(parseFloat(g))&&(g=g.replace(/[^0-9.-]/,""));return parseFloat(g)}};
