...
a.os.ios?a("#afui").addClass("ios"):a.os.tizen?a("#afui").addClass("tizen"):a.os.fennec&&(a("#afui").addClass("firefox"),f.animateHeaders=!1));if(a.os.ios){a("head").find("#iosBlurrHack").remove();var b;b="-webkit-backface-visibility: hidden;-webkit-perspective:1000;";a("head").append("<style id='iosBlurrHack'>#afui .y-scroll > *, #afui .x-scroll > * {"+b+"}</style>")}else a.os.android&&!a.os.androidICS?f.transitionTime="150ms":a.os.fennec&&f.ready(function(){window.addEventListener("deviceorientation",
function(){var b=numOnly(a("#header").css("height"))+numOnly(a("#navbar").css("height"));a("#content").css("height",window.innerHeight-b)})})}var f=this;this.availableTransitions={};this.availableTransitions["default"]=this.availableTransitions.none=this.noTransition;if("function"===typeof define&&define.amd||"undefined"!==typeof module&&module.exports)f.autoLaunch=!1;var c=function(){var c=document.getElementById("afui");if(null===c){c=document.createElement("div");c.id="afui";for(var g=document.body;g&&
g.firstChild;)c.appendChild(g.firstChild);a(document.body).prepend(c)}f.isIntel="intel"in window&&window.intel&&window.intel.xdk&&"app"in window.intel.xdk;a.os.supportsTouch&&a.touchLayer(c);b()};if("complete"===document.readyState||"loaded"===document.readyState)if(c(),f.init)f.autoBoot();else a(window).one("afui:init",function(){f.autoBoot()});else a(document).ready(function(){c();if(f.init)f.autoBoot();else a(window).one("afui:init",function(){f.autoBoot()})},!1);"intel"in window||(window.intel=
...
this.content=af.query("#content").get(0);this.header=af.query("#header").get(0);this.menu=af.query("#menu").get(0);this.viewportContainer.on("click","a",function(a){b.useInternalRouting&&k(a,a.currentTarget)});var f=null;a.bind(a.touchLayer,"enter-edit",function(a){f=a});a.os.android&&!a.os.androidICS&&(a.bind(a.touchLayer,"enter-edit-reshape",function(){var c=a(f),e=c.closest(b.activeDiv);if(e&&0<e.size()){var d=c.offset(),e=e.offset();d.bottom>e.bottom&&d.height<e.height&&b.scrollingDivs[b.activeDiv.id].scrollToItem(c,
...
c),a.query("#afui #content").addClass("hasMenu"+c),a.query("#afui #navbar").addClass("hasMenu"+c),a.query("#afui #menu").addClass("hasMenu"+c),a.query("#afui #aside_menu").addClass(c));0<a.query("aside").length&&a.query("#afui #header, #afui #content, #afui #navbar").addClass("hasAside");a.query("#afui #menu").addClass("tabletMenu");a.ui.splitview&&window.innerWidth>parseInt(a.ui.handheldMinWidth,10)&&(a.ui.sideMenuWidth=a("#menu").css("width")+"px");var c=b.getPanelId(m),e=null!==c&&c==="#"+b.firstDiv.id;
0<c.length&&b.loadDefaultHash&&!e?b.loadContent(m,!0,!1,"none"):(l="#"+b.firstDiv.id,b.firstDiv.style.display="block",b.parsePanelFunctions(b.firstDiv),b.loadContentData(b.firstDiv),a.query("#header .backButton").css("visibility","hidden"),("true"===b.firstDiv.getAttribute("data-modal")||"true"===b.firstDiv.getAttribute("modal"))&&b.showModal(b.firstDiv.id));b.launchCompleted=!0;a.query("#afui #splashscreen").remove();a.os.fennec?a(document).trigger("afui:ready"):setTimeout(function(){a(document).trigger("afui:ready")})},
