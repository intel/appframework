...
this.isLaunching=!0;var b=this;this.viewportContainer=af.query("#afui");this.navbar=af.query("#navbar").get(0);this.content=af.query("#content").get(0);this.header=af.query("#header").get(0);this.menu=af.query("#menu").get(0);this.viewportContainer.on("click","a",function(a){b.useInternalRouting&&k(a,a.currentTarget)});var f=null;a.bind(a.touchLayer,"enter-edit",function(a){f=a});a.os.android&&(a.bind(a.touchLayer,"enter-edit-reshape",function(){var c=a(f),d=c.closest(b.activeDiv);if(d&&0<d.size())if(a.os.androidICS)paddingBottom=
d.offset().bottom-c.offset().bottom,b.scrollingDivs[b.activeDiv.id].scrollBy({x:0,y:paddingBottom});else{var e=c.offset(),d=d.offset();e.bottom>d.bottom&&e.height<d.height&&b.scrollingDivs[b.activeDiv.id].scrollToItem(c,"bottom")}}),a.bind(a.touchLayer,"exit-edit-reshape",function(){b.activeDiv&&(b.activeDiv.id&&b.scrollingDivs.hasOwnProperty(b.activeDiv.id))&&b.scrollingDivs[b.activeDiv.id].setPaddings(0,0)}));this.navbar||(this.navbar=a.create("div",{id:"navbar"}).get(0),this.viewportContainer.append(this.navbar));
...
{id:"modalTransContainer"}).appendTo(c).get(0);this.viewportContainer.append(c);this.scrollingDivs.modal_container=a.query("#modalContainer").scroller({scrollBars:!0,vertical:!0,vScrollCSS:"afScrollbar",lockBounce:this.lockPageBounce});this.modalWindow=c;for(var e={},c=this.viewportContainer.get(0).querySelectorAll(".panel"),d=0;d<c.length;d++){var h=c[d],n=h,w=h.previousSibling;h.parentNode&&"content"!==h.parentNode.id?(n.getAttribute("selected")&&(this.firstDiv=h),this.addDivAndScroll(n),a.query("#content").append(h)):
h.parsedContent||(h.parsedContent=1,n.getAttribute("selected")&&(this.firstDiv=h),this.addDivAndScroll(h),a(h).insertAfter(w));h.getAttribute("data-defer")&&(e[h.id]=h.getAttribute("data-defer"));this.firstDiv||(this.firstDiv=h);h=null}var c=null,s=!1,u=Object.keys(e).length,c=function(c){a.ajax({url:intel.xdk.webRoot+e[c],success:function(d){0<d.length&&(b.updatePanel(c,d),b.parseScriptTags(a.query("#"+c).get(0)));t++;t>=u&&(s=!1,a(document).trigger("defer:loaded"))},error:function(){console.log("Error with deferred load "+
intel.xdk.webRoot+e[c]);t++;t>=u&&(s=!1,a(document).trigger("defer:loaded"))}})};if(0<u){var s=!0,t=0,q;for(q in e)c(q)}if(this.firstDiv)if(this.activeDiv=this.firstDiv,this.scrollingDivs[this.activeDiv.id]&&this.scrollingDivs[this.activeDiv.id].enable(),q=function(){a.query("#navbar").append(a.create("footer",{id:"defaultNav"}).append(a.query("#navbar").children()));b.defaultFooter="defaultNav";b.prevFooter=a.query("#defaultNav");b.updateNavbarElements(b.prevFooter);var c=a.query("nav").get(0);c&&
