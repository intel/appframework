...
this.unbind(a,c):this.undelegate(c,a,d)};m.fn.trigger=function(a,c,d){"string"===typeof a&&(d=d||{},a=p(a),d.ns=a.ns,a=m.Event(a.e,d));a.data=c;c=0;for(d=this.length;c<d;c++)this[c].dispatchEvent(a);return this};m.Event=function(a,c){var d=r.createEvent("Events"),b=!0;if(c)for(var e in c)"bubbles"===e?b=!!c[e]:d[e]=c[e];d.initEvent(a,b,!0,null,null,null,null,null,null,null,null,null,null,null,null);return d};m.bind=function(a,c,d){if(a){a.__events||(a.__events={});m.isArray(c)||(c=[c]);for(var b=
0;b<c.length;b++)a.__events[c[b]]||(a.__events[c[b]]=[]),a.__events[c[b]].push(d)}};m.trigger=function(a,c,d){if(a){var b=!0;if(!a.__events)return b;m.isArray(c)||(c=[c]);m.isArray(d)||(d=[]);for(var e=0;e<c.length;e++)if(a.__events[c[e]])for(var p=a.__events[c[e]].slice(0),f=0;f<p.length;f++)m.isFunction(p[f])&&!1===p[f].apply(a,d)&&(b=!1);return b}};m.unbind=function(a,c,d){if(a.__events)if(c==q)delete a.__events;else{m.isArray(c)||(c=[c]);for(var b=0;b<c.length;b++)if(a.__events[c[b]])for(var e=
a.__events[c[b]],p=0;p<e.length;p++)if(d==q&&delete e[p],e[p]===d){e.splice(p,1);break}}};m.proxy=function(a,c,d){return function(){return d?a.apply(c,d):a.apply(c,arguments)}};var W=function(a,c){for(var d=0;d<a.length;d++)A(a[d],c)};m.cleanUpContent=function(a,c,d){if(a){var b=a.childNodes;b&&0<b.length&&m.asap(W,{},[K.apply(b,[0]),d]);c&&s(a,d)}};var H=[],M=[],N=[];m.asap=function(c,d,b){if(!m.isFunction(c))throw"$.asap - argument is not a valid function";H.push(c);M.push(d?d:{});N.push(b?b:[]);
a.postMessage("afm-asap","*")};a.addEventListener("message",function(c){c.source===a&&"afm-asap"===c.data&&(c.stopPropagation(),0<H.length&&H.shift().apply(M.shift(),N.shift()))},!0);var O={};m.parseJS=function(c){if(c){if("string"===typeof c){var d=r.createElement("div");D?MSApp.execUnsafeLocalFunction(function(){d.innerHTML=c}):d.innerHTML=c;c=d}var b=c.getElementsByTagName("script");c=null;for(var e=0;e<b.length;e++)if(0<b[e].src.length&&!O[b[e].src]&&!D){var p=r.createElement("script");p.type=
b[e].type;p.src=b[e].src;r.getElementsByTagName("head")[0].appendChild(p);O[b[e].src]=1;p=null}else a.eval(b[e].innerHTML)}};"click keydown keyup keypress submit load resize change select error".split(" ").forEach(function(a){m.fn[a]=function(c){return c?this.bind(a,c):this.trigger(a)}});["focus","blur"].forEach(function(a){m.fn[a]=function(c){if(0!==this.length){if(c)this.bind(a,c);else for(c=0;c<this.length;c++)try{this[c][a]()}catch(d){}return this}}});m.Deferred=function(){return{reject:function(){},
