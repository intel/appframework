...
{id:"modalTransContainer"}).appendTo(c).get(0);this.viewportContainer.append(c);this.scrollingDivs.modal_container=a.query("#modalContainer").scroller({scrollBars:!0,vertical:!0,vScrollCSS:"afScrollbar",lockBounce:this.lockPageBounce});this.modalWindow=c;for(var d={},c=this.viewportContainer.get(0).querySelectorAll(".panel"),f=0;f<c.length;f++){var g=c[f],n=g,y=g.previousSibling;g.parentNode&&"content"!==g.parentNode.id?(n.getAttribute("selected")&&(this.firstDiv=g),this.addDivAndScroll(n),a.query("#content").append(g)):
g.parsedContent||(g.parsedContent=1,n.getAttribute("selected")&&(this.firstDiv=g),this.addDivAndScroll(g),a(g).insertAfter(y));g.getAttribute("data-defer")&&(d[g.id]=g.getAttribute("data-defer"));this.firstDiv||(this.firstDiv=g);g=null}var c=null,u=!1,E=Object.keys(d).length,c=function(c){a.ajax({url:intel.xdk.webRoot+d[c],success:function(d){0<d.length&&(b.updatePanel(c,d),b.parseScriptTags(a.query("#"+c).get(0)));z++;z>=E&&(u=!1,a(document).trigger("defer:loaded"))},error:function(){console.log("Error with deferred load "+
intel.xdk.webRoot+d[c]);z++;z>=E&&(u=!1,a(document).trigger("defer:loaded"))}})};if(0<E){var u=!0,z=0,s;for(s in d)c(s)}if(this.firstDiv)if(this.activeDiv=this.firstDiv,this.scrollingDivs[this.activeDiv.id]&&this.scrollingDivs[this.activeDiv.id].enable(),s=function(){a.query("#navbar").append(a.create("footer",{id:"defaultNav"}).append(a.query("#navbar").children()));b.defaultFooter="defaultNav";b.prevFooter=a.query("#defaultNav");b.updateNavbarElements(b.prevFooter);var c=a.query("nav").get(0);c&&
