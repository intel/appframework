...
!0,this.onScrollStart))this.onScrollStart();b=this.getCSSMatrix(this.el);this.lastScrollInfo.top=numOnly(b.f);this.lastScrollInfo.left=numOnly(b.e);this.recalculateDeltaY(this.lastScrollInfo);this.recalculateDeltaX(this.lastScrollInfo);this.checkYboundary(this.lastScrollInfo);this.elementInfo.hasHorScroll&&this.checkXboundary(this.lastScrollInfo);b=0<this.lastScrollInfo.y&&0<this.lastScrollInfo.deltaY;var f=this.lastScrollInfo.y<-this.elementInfo.maxTop&&0>this.lastScrollInfo.deltaY,e;if(b||f)d=b?
this.lastScrollInfo.y:-this.lastScrollInfo.y-this.elementInfo.maxTop,d=(this.container.clientHeight-d)/this.container.clientHeight,0.5>d&&(d=0.5),e=0,b&&0<this.lastScrollInfo.top||f&&this.lastScrollInfo.top<-this.elementInfo.maxTop?e=this.lastScrollInfo.top:f&&(e=-this.elementInfo.maxTop),f=this.lastScrollInfo.deltaY*d,1>Math.abs(this.lastScrollInfo.deltaY*d)&&(f=b?1:-1),this.lastScrollInfo.y=e+f;if(this.elementInfo.hasHorScroll&&(b=0<this.lastScrollInfo.x&&0<this.lastScrollInfo.deltaX,f=this.lastScrollInfo.x<
-this.elementInfo.maxLeft&&0>this.lastScrollInfo.deltaX,b||f))d=b?this.lastScrollInfo.x:-this.lastScrollInfo.x-this.elementInfo.maxLeft,d=(this.container.clientWidth-d)/this.container.clientWidth,0.5>d&&(d=0.5),e=0,b&&0<this.lastScrollInfo.left||f&&this.lastScrollInfo.left<-this.elementInfo.maxLeft?e=this.lastScrollInfo.left:f&&(e=-this.elementInfo.maxLeft),f=this.lastScrollInfo.deltaX*d,1>Math.abs(this.lastScrollInfo.deltaX*d)&&(f=b?1:-1),this.lastScrollInfo.x=e+f;this.lockBounce&&!this.refresh&&
...
h.prototype.calculateMovement=function(a,b){var f={top:0,left:0,speedY:0,speedX:0,absSpeedY:0,absSpeedX:0,deltaY:0,deltaX:0,absDeltaY:0,absDeltaX:0,y:0,x:0,duration:0},e=b?this.firstEventInfo:this.lastEventInfo,g=b?a.pageX:a.touches[0].pageX,k=b?a.pageY:a.touches[0].pageY,h=b?a.time:a.timeStamp;f.deltaY=this.elementInfo.hasVertScroll?k-e.pageY:0;f.deltaX=this.elementInfo.hasHorScroll?g-e.pageX:0;f.time=h;f.duration=h-e.time;return f};h.prototype.calculateTarget=function(a){a.y=this.lastScrollInfo.y+
a.deltaY;a.x=this.lastScrollInfo.x+a.deltaX;0<Math.abs(a.deltaY)&&(a.y+=(0<a.deltaY?1:-1)*this.elementInfo.divHeight*this.androidPerfHack);0<Math.abs(a.deltaX)&&(a.x+=(0<a.deltaX?1:-1)*this.elementInfo.divWidth*this.androidPerfHack)};h.prototype.checkYboundary=function(a){var b=this.container.clientHeight/2,f=this.elementInfo.maxTop+b;if(a.y>b)a.y=b;else if(-a.y>f)a.y=-f;else return;this.recalculateDeltaY(a)};h.prototype.checkXboundary=function(a){var b=this.container.clientWidth/2,f=this.elementInfo.maxLeft+
b;if(a.x>b)a.x=b;else if(-a.x>f)a.x=-f;else return;this.recalculateDeltaX(a)};h.prototype.recalculateDeltaY=function(a){var b=Math.abs(a.deltaY);a.deltaY=a.y-a.top;var f=Math.abs(a.deltaY);a.duration=a.duration*f/b};h.prototype.recalculateDeltaX=function(a){var b=Math.abs(a.deltaX);a.deltaX=a.x-a.left;var f=Math.abs(a.deltaX);a.duration=a.duration*f/b};h.prototype.hideRefresh=function(b){var d=this;if(!this.preventHideRefresh){var f=function(){d.setRefreshContent("Pull to Refresh");a.trigger(d,"refresh-finish")};
...
a.absSpeedY/8E-4,a.speedY=a.deltaY/a.duration,a.absSpeedY=Math.abs(a.speedY),0.08>a.absSpeedY||5>a.absDeltaY)a.deltaY=a.absDeltaY=a.duration=a.speedY=a.absSpeedY=0}else if(a.absDeltaX){if(a.deltaX+=(0>a.deltaX?-1:1)*a.absSpeedX*a.absSpeedX/0.0016,a.absDeltaX=Math.abs(a.deltaX),a.duration=a.absSpeedX/8E-4,a.speedX=a.deltaX/a.duration,a.absSpeedX=Math.abs(a.speedX),0.08>a.absSpeedX||5>a.absDeltaX)a.deltaX=a.absDeltaX=a.duration=a.speedX=a.absSpeedX=0}else a.duration=0};h.prototype.onTouchEnd=function(){if(null!==
this.currentScrollingObject&&this.moved){this.finishScrollingObject=this.currentScrollingObject;this.currentScrollingObject=null;var b=this.calculateMovement(this.lastEventInfo,!0);this.androidFormsMode||this.setMomentum(b);this.calculateTarget(b);var d=this.getCSSMatrix(this.el);b.top=numOnly(d.f);b.left=numOnly(d.e);this.checkYboundary(b);this.elementInfo.hasHorScroll&&this.checkXboundary(b);d=!this.preventPullToRefresh&&(b.top>this.refreshHeight||b.y>this.refreshHeight);this.fireRefreshRelease(d,
0<b.top);if(this.refresh&&d)b.y=this.refreshHeight,b.duration=325;else if(0<=b.y)b.y=0,0<=b.top&&(b.duration=325);else if(-b.y>this.elementInfo.maxTop||0===this.elementInfo.maxTop)b.y=-this.elementInfo.maxTop,-b.top>this.elementInfo.maxTop&&(b.duration=325);if(this.elementInfo.hasHorScroll)if(0<=b.x)b.x=0,0<=b.left&&this.refresh&&(b.duration=325);else if(-b.x>this.elementInfo.maxLeft||0===this.elementInfo.maxLeft)b.x=-this.elementInfo.maxLeft,-b.left>this.elementInfo.maxLeft&&this.refresh&&(b.duration=
...
50)}}};h.prototype.setFinishCalback=function(b){var d=this;this.scrollingFinishCB=setTimeout(function(){d.hideScrollbars();a.trigger(a.touchLayer,"scrollend",[d.el]);a.trigger(d,"scrollend",[d.el]);d.isScrolling=!1;d.elementInfo=null;d.infinite&&d.infiniteTriggered&&a.trigger(d,"infinite-scroll-end")},b)};h.prototype.startFormsMode=function(){if(!this.blockFormsFix){var b=this.getCSSMatrix(this.el);this.refreshSafeKeep=this.refresh;this.refresh=!1;this.androidFormsMode=!0;this.el.style[a.feat.cssPrefix+
