...
(b.modalReference_.children().eq(0).append(a("#modalHeader header")),a(b.modalReference_.get(0).childNodes[0]).append(e.children().eq(0).contents()),b.modalReference_.children().eq(0).append(a("#modalFooter footer")))},this.transitionTime)},updatePanel:function(b,e){b="#"+b.replace("#","");var c=a.query(b).get(0);if(c){var d=a.create("div",{html:e}),d=d.children(".panel")&&0<d.children(".panel").length?d.children(".panel").get(0):d.get(0);c.getAttribute("js-scrolling")&&("yes"===c.getAttribute("js-scrolling").toLowerCase()||
...
f&&a(b).children().eq(0).addClass("afScrollPanel"));return g.id},scrollToTop:function(a,e){e=e||"300ms";a=a.replace("#","");this.scrollingDivs[a]&&this.scrollingDivs[a].scrollToTop(e)},scrollToBottom:function(a,e){a=a.replace("#","");this.scrollingDivs[a]&&this.scrollingDivs[a].scrollToBottom(e)},parsePanelFunctions:function(b,e,c){var d=b.getAttribute("data-footer"),f=b.getAttribute("data-header");d&&"none"===d.toLowerCase()?(this.toggleNavMenu(!1),d=!1):this.toggleNavMenu(!0);d&&this.customFooter!==
d?(this.customFooter=d,this.updateNavbarElements(d)):d!==this.customFooter&&(this.customFooter&&this.updateNavbarElements(this.defaultFooter),this.customFooter=!1);f&&"none"===f.toLowerCase()?(this.toggleHeaderMenu(!1),f=!1):this.toggleHeaderMenu(!0);f&&this.customHeader!==f?(this.customHeader=f,this.updateHeaderElements(f,c)):f!==this.customHeader&&(this.customHeader&&this.updateHeaderElements(this.defaultHeader,c),this.customHeader=!1);d=a(b).find("footer");0<d.length&&(this.customFooter=b.id,d.data("parent",
...
this.activeDiv.id&&b===this.ajaxUrl)){var h="url"+g(b),k=this;-1===b.indexOf("http")&&(b=intel.xdk.webRoot+b);var l=new XMLHttpRequest;f&&"object"!==typeof f&&(f=document.createElement("a"),f.setAttribute("data-persist-ajax",!0));f=f||document.createElement("a");l.onreadystatechange=function(){if(4===l.readyState&&200===l.status){this.doingTransition=!1;var g,n=!1;g=a.query("#"+h);var s="",A=function(a){(a=k.scrollingDivs[a])&&a.refresh&&a.hideRefresh()};if(0<g.length)A(h),k.updatePanel(h,l.responseText),
g.get(0).setAttribute("data-title",f.title?f.title:b);else if(f.getAttribute("data-persist-ajax")||k.isAjaxApp){var q="true"===f.getAttribute("data-pull-scroller")?!0:!1;g=q?function(){setTimeout(function(){f.refresh=!0;k.loadContent(b,e,c,d,f);f.refresh=!1},500)}:null;var r=a(l.responseText);r.hasClass("panel")?(s=r.get(0).className,h=r.attr("id"),r=r.get(0).innerHTML):(s=r.className,r=r.html());A(h);0<a("#"+h).length?k.updatePanel("#"+h,r):0<a("div.panel[data-crc='"+h+"']").length?(k.updatePanel(a("div.panel[data-crc='"+
h+"']").get(0).id,r),h=a("div.panel[data-crc='"+h+"']").get(0).id):h=k.addContentDiv(h,l.responseText,f.title?f.title:b,q,g)}else k.updatePanel("afui_ajax",l.responseText),a.query("#afui_ajax").attr("data-title",f.title?f.title:b),k.loadContent("#afui_ajax",e,c,d),n=!0;A=document.createElement("div");a(A).html(l.responseText);k.parseScriptTags(A);if(n)k.showLoading&&k.hideMask();else{if(s){n=s.split(" ");for(s=0;s<n.length;s++)"panel"!==n[s].trim()&&a("#"+h).addClass(n[s])}k.loadContent("#"+h,e,c,
...
e,c,d)},launch:function(){if(!1===this.hasLaunched||this.launchCompleted)this.hasLaunched=!0;else{if(this.isLaunching)return!0;this.isLaunching=!0;var b=this;this.viewportContainer=af.query("#afui");this.navbar=af.query("#navbar").get(0);this.content=af.query("#content").get(0);this.header=af.query("#header").get(0);this.menu=af.query("#menu").get(0);this.viewportContainer.on("click","a",function(a){b.useInternalRouting&&h(a,a.currentTarget)});var e=null;a.bind(a.touchLayer,"enter-edit",function(a){e=
a});a.os.android&&(a.bind(a.touchLayer,"enter-edit-reshape",function(){var c=a(e),d=c.closest(b.activeDiv);if(d&&0<d.size())if(a.os.androidICS){var f=b.scrollingDivs[b.activeDiv.id],g=f.scrollTop;f.scrollBy({x:0,y:(c.offset().top-d.offset().top+g)/2})}else if(a.os.android||a.os.blackberry)f=c.offset(),d=d.offset(),f.bottom>d.bottom&&f.height<d.height&&b.scrollingDivs[b.activeDiv.id].scrollToItem(c,"bottom")}),a.bind(a.touchLayer,"exit-edit-reshape",function(){b.activeDiv&&(b.activeDiv.id&&b.scrollingDivs.hasOwnProperty(b.activeDiv.id))&&
...
f=0;f<c.length;f++){var g=c[f],n=g,y=g.previousSibling;g.parentNode&&"content"!==g.parentNode.id?(n.getAttribute("selected")&&(this.firstDiv=g),this.addDivAndScroll(n),a.query("#content").append(g)):g.parsedContent||(g.parsedContent=1,n.getAttribute("selected")&&(this.firstDiv=g),this.addDivAndScroll(g),a(g).insertAfter(y));g.getAttribute("data-defer")&&(d[g.id]=g.getAttribute("data-defer"));this.firstDiv||(this.firstDiv=g);g=null}var c=null,u=!1,F=Object.keys(d).length,c=function(c){a.ajax({url:intel.xdk.webRoot+
d[c],success:function(d){0<d.length&&(b.updatePanel(c,d),b.parseScriptTags(a.query("#"+c).get(0)));z++;z>=F&&(u=!1,a(document).trigger("defer:loaded"))},error:function(){console.log("Error with deferred load "+intel.xdk.webRoot+d[c]);z++;z>=F&&(u=!1,a(document).trigger("defer:loaded"))}})};if(0<F){var u=!0,z=0,s;for(s in d)c(s)}if(this.firstDiv)if(this.activeDiv=this.firstDiv,this.scrollingDivs[this.activeDiv.id]&&this.scrollingDivs[this.activeDiv.id].enable(),s=function(){a.query("#navbar").append(a.create("footer",
...
c),a.query("#afui #aside_menu").addClass(c));0<a.query("aside").length&&a.query("#afui #header, #afui #content, #afui #navbar").addClass("hasAside");a.query("#afui #menu").addClass("tabletMenu");a.ui.splitview&&window.innerWidth>parseInt(a.ui.handheldMinWidth,10)&&(a.ui.sideMenuWidth=a("#menu").css("width")+"px");var c=b.getPanelId(l),d=null!==c&&c==="#"+b.firstDiv.id;0<c.length&&b.loadDefaultHash&&!d?b.loadContent(l,!0,!1,"none"):(k="#"+b.firstDiv.id,b.firstDiv.style.display="block",b.parsePanelFunctions(b.firstDiv),
b.loadContentData(b.firstDiv),a.query("#header .backButton").css("visibility","hidden"),("true"===b.firstDiv.getAttribute("data-modal")||"true"===b.firstDiv.getAttribute("modal"))&&b.showModal(b.firstDiv.id));b.launchCompleted=!0;a.query("#afui #splashscreen").remove();a.os.fennec?a(document).trigger("afui:ready"):setTimeout(function(){a(document).trigger("afui:ready")})},u)a(document).one("defer:loaded",s);else s();else setTimeout(function(){a(document).trigger("afui:ready")});a.bind(b,"content-loaded",
function(){if(0<b.loadContentQueue.length){var a=b.loadContentQueue.splice(0,1)[0];b.loadContent(a[0],a[1],a[2],a[3],a[4])}});(window.navigator.standalone||this.isIntel)&&this.blockPageScroll();this.enableTabBar();this.topClickScroll()}},topClickScroll:function(){var a=this;document.getElementById("header").addEventListener("click",function(e){15>=e.clientY&&"h1"===e.target.nodeName.toLowerCase()&&a.scrollingDivs[a.activeDiv.id].scrollToTop("100")})},blockPageScroll:function(){a.query("#afui #header").bind("touchmove",
