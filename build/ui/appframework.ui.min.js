...
vertical:!0,vScrollCSS:"afScrollbar",lockBounce:this.lockPageBounce});this.modalWindow=c;for(var d={},c=this.viewportContainer.get(0).querySelectorAll(".panel"),f=0;f<c.length;f++){var g=c[f],h=g,p=g.previousSibling;g.parentNode&&"content"!==g.parentNode.id?(h.getAttribute("selected")&&(this.firstDiv=g),this.addDivAndScroll(h),a.query("#content").append(g)):g.parsedContent||(g.parsedContent=1,h.getAttribute("selected")&&(this.firstDiv=g),this.addDivAndScroll(g),a(g).insertAfter(p));g.getAttribute("data-defer")&&
(d[g.id]=g.getAttribute("data-defer"));this.firstDiv||(this.firstDiv=g);g=null}var c=null,t=!1,E=Object.keys(d).length,c=function(c){a.ajax({url:intel.xdk.webRoot+d[c],success:function(d){0<d.length&&(b.updatePanel(c,d),b.parseScriptTags(a.query("#"+c).get(0)));C++;C>=E&&(t=!1,a(document).trigger("defer:loaded"))},error:function(){console.log("Error with deferred load "+intel.xdk.webRoot+d[c]);C++;C>=E&&(t=!1,a(document).trigger("defer:loaded"))}})};if(0<E){var t=!0,C=0,u;for(u in d)c(u)}if(this.firstDiv)if(this.activeDiv=
this.firstDiv,this.scrollingDivs[this.activeDiv.id]&&this.scrollingDivs[this.activeDiv.id].enable(),u=function(){a.query("#navbar").append(a.create("footer",{id:"defaultNav"}).append(a.query("#navbar").children()));b.defaultFooter="defaultNav";b.prevFooter=a.query("#defaultNav");b.updateNavbarElements(b.prevFooter);var c=a.query("nav").get(0);c&&(b.defaultMenu=a(c),b.updateSideMenuElements(b.defaultMenu),b.prevMenu=b.defaultMenu);if(c=a.query("aside").get(0))b.defaultAside=a(c),b.updateAsideElements(b.defaultAside),
...
"px");var c=b.getPanelId(m),d=null!==c&&c==="#"+b.firstDiv.id;0<c.length&&b.loadDefaultHash&&!d?b.loadContent(m,!0,!1,"none"):(l="#"+b.firstDiv.id,b.firstDiv.style.display="block",b.parsePanelFunctions(b.firstDiv),b.loadContentData(b.firstDiv),a.query("#header .backButton").css("visibility","hidden"),("true"===b.firstDiv.getAttribute("data-modal")||"true"===b.firstDiv.getAttribute("modal"))&&b.showModal(b.firstDiv.id));b.launchCompleted=!0;a.query("#afui #splashscreen").remove();setTimeout(function(){a(document).trigger("afui:ready")})},
