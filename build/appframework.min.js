...
b,c){if(a&&a.length!=h&&0===a.length)return this;if(e.isArray(a)||e.isObject(a))a=e(a);var d;b&&e(this).add(b);for(b=0;b<this.length;b++)if(a.length&&"string"!==typeof a)a=e(a),v(a,this[b],c);else{var g=Y.test(a)?e(a):void 0;if(g==h||0===g.length)g=k.createTextNode(a);if(g instanceof r)for(var D=0,R=g.length;D<R;D++)d=g[D],d.nodeName!=h&&"script"===d.nodeName.toLowerCase()&&(!d.type||"text/javascript"===d.type.toLowerCase())?f.eval(d.innerHTML):v(e(d),this[b],c);else c!=h?this[b].insertBefore(g,this[b].firstChild):
...
l.call?n(l,f):p.resolve(f)},callRejected:function(f){m&&m.call?n(m,f):p.reject(f)}}};f.defer=function(){var f=0,m,p=[],n=function(s){f=1;m=s;p.forEach(function(f){f.callFulfilled(m)})},q=function(s){f=2;m=s;p.forEach(function(f){f.callRejected(m)})},C=function(s,t){var n=y(s,t);p.push(n);1===f?n.callFulfilled(m):2===f&&n.callRejected(m);return n.promise},z=function(f){var l;try{var m=f&&f.then;l=null!==f&&"object"===typeof f&&"function"===typeof m?m.bind(f):void 0}catch(p){q(p);return}if(l){f=l;l=
