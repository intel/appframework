...
c),e.prop("id",r),e.addClass("panel"));a(k.activeDiv).closest(".pages").append(e);e.attr("data-crc",r);k.showLoading&&k.hideMask();k.loadContent("#"+e.prop("id"),b,f,g,h)}).fail(function(a){k.showLoading&&k.hideMask();console.log("Error with ajax request",a)})},runTransition:function(c,b,e,f,g){c||(c="slide");-1!==c.indexOf(":back")&&(c=c.replace(":back",""),f=!0);var k=this,h=f?b:e;b=f?e:b;e=!1;-1!==c.indexOf("-reveal")&&(c=c.replace("-reveal",""),e=!0);a(h).css("zIndex","10");a(b).css("zIndex",
"1");a(g).css("zIndex","1").addClass("active");var q=a(b).animation().remove(c+"-in");!e&&q?(f&&q.reverse(),q.end(function(){f?(this.classList.add("active"),a(this).trigger("panelload")):(this.classList.remove("active"),a(this).trigger("panelunload"));k.doingTransition=!1}).run(c+"-out")):f?(a(b).trigger("panelload"),a(b).addClass("activeDiv")):a(b).trigger("panelunload");h=a(h).animation().remove(c+"-out");f&&h.reverse();h.end(function(){k.doingTransition=!1;f?(g&&a(g).css("zIndex","10"),this.classList.remove("active"),
a(this).trigger("panelunload")):(this.classList.add("active"),a(this).trigger("panelload"),a(g).trigger("panelload"))}).run(c+"-in")},runViewTransition:function(c,b,e,f,g){b.addClass("active");a(f).addClass("active");"none"===c?(this.doingTransition=!1,setTimeout(function(){e.removeClass("active");b.addClass("active");a(f).addClass("active")},50)):this.runTransition(c,e,b,g,f)},updateViewHistory:function(a,b,e,f){var g=this.views[a.prop("id")];g||(g=this.views[a.prop("id")]=[]);1<=g.length&&g[g.length-
...
d=a(".view[data-default='true']");if(0===d.length&&(d=a(".view").eq(0),0===d.length))throw"You need to create a view";d.addClass("active");this.views[d.prop("id")]=[];var e=window.location.hash,g=0===d.find(".panel[data-selected='true']").length?d.find(".panel").eq(0):d.find(".panel[data-selected='true']");g.addClass("active");this.activeDiv=g.get(0);this.views[d.prop("id")].push({target:g.get(0),transition:this.transitionType});this.defaultPanel=g.get(0);this.loadContentData(g.get(0),d,!1,!0);this.updateHash(g.get(0).id);
...
".swipe-reveal",function(c,d,m){m.preventDefault();h=a(c.target).closest(".swipe-content");f=h.closest(".swipe-reveal").find(".swipe-hidden").width();0!==a.getCssMatrix(c.target).e&&(g=d.x2+f,h.bind("touchmove",e),h.one("touchend",function(){h.unbind("touchmove",e);1-Math.abs(b)/f>a.afui.swipeThreshold&&(f=0);h.transition().keep().end(function(){h=f=null}).run("translate3d("+-f+"px,0px,0)","100ms")}))});var e=function(a){a=-(g-a.touches[0].pageX);a<-f?a="-"+f:0<a&&(a=0);b=a;h.cssTranslate(a+"px,0")}})(jQuery);
