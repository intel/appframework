/*
 ...
 @api private
*/
if(!window.af||"function"!==typeof af){var af=function(c){function n(F,c,b){var a=t.createDocumentFragment();if(b){for(b=F.length-1;0<=b;b--)a.insertBefore(F[b],a.firstChild);c.insertBefore(a,c.firstChild)}else{for(b=0;b<F.length;b++)a.appendChild(F[b]);c.appendChild(a)}}function l(c){return c in H?H[c]:H[c]=RegExp("(^|\\s)"+c+"(\\s|$)")}function k(c){for(var b=0;b<c.length;b++)c.indexOf(c[b])!=b&&(c.splice(b,1),b--);return c}function g(c,b){var a=[];if(c==p)return a;for(;c;c=c.nextSibling)1==c.nodeType&&
...
c)};var x={},O=1;m.event={add:s,remove:G};m.fn.bind=function(b,c){for(var a=0;a<this.length;a++)s(this[a],b,c);return this};m.fn.unbind=function(b,c){for(var a=0;a<this.length;a++)G(this[a],b,c);return this};m.fn.one=function(b,c){return this.each(function(a,d){s(this,b,c,null,function(b,c){return function(){G(d,c,b);return b.apply(d,arguments)}})})};var R=function(){return!0},S=function(){return!1},Q={preventDefault:"isDefaultPrevented",stopImmediatePropagation:"isImmediatePropagationStopped",stopPropagation:"isPropagationStopped"};
m.fn.delegate=function(b,c,a){for(var d=0;d<this.length;d++)D(this[d],c,a,b);return this};m.fn.undelegate=function(b,c,a){for(var d=0;d<this.length;d++)G(this[d],c,a,b);return this};m.fn.on=function(b,c,a){return c===p||m.isFunction(c)?this.bind(b,c):this.delegate(c,b,a)};m.fn.off=function(b,c,a){return c===p||m.isFunction(c)?this.unbind(b,c):this.undelegate(c,b,a)};m.fn.trigger=function(b,c,a){"string"==typeof b&&(b=m.Event(b,a));b.data=c;for(c=0;c<this.length;c++)this[c].dispatchEvent(b);return this};
...
b.__events[c[h]].slice(0),f=0;f<e.length;f++)m.isFunction(e[f])&&!1===e[f].apply(b,a)&&(d=!1);return d}};m.unbind=function(b,c,a){if(b.__events){m.isArray(c)||(c=[c]);for(var d=0;d<c.length;d++)if(b.__events[c[d]])for(var h=b.__events[c[d]],e=0;e<h.length;e++)if(a==p&&delete h[e],h[e]==a){h.splice(e,1);break}}};m.proxy=function(b,c,a){return function(){return a?b.apply(c,a):b.apply(c,arguments)}};var V=function(b,c){for(var a=0;a<b.length;a++)z(b[a],c)};m.cleanUpContent=function(b,c,a){if(b){var d=
b.childNodes;d&&0<d.length&&m.asap(V,{},[J.apply(d,[0]),a]);c&&E(b,a)}};var I=[],L=[],M=[];m.asap=function(b,a,d){if(!m.isFunction(b))throw"$.asap - argument is not a valid function";I.push(b);L.push(a?a:{});M.push(d?d:[]);c.postMessage("afm-asap","*")};c.addEventListener("message",function(b){b.source==c&&"afm-asap"==b.data&&(b.stopPropagation(),0<I.length&&I.shift().apply(L.shift(),M.shift()))},!0);var N={};m.parseJS=function(b){if(b){if("string"==typeof b){var a=t.createElement("div");C?MSApp.execUnsafeLocalFunction(function(){a.innerHTML=
b}):a.innerHTML=b;b=a}var d=b.getElementsByTagName("script");b=null;for(var h=0;h<d.length;h++)if(0<d[h].src.length&&!N[d[h].src]&&!C){var e=t.createElement("script");e.type=d[h].type;e.src=d[h].src;t.getElementsByTagName("head")[0].appendChild(e);N[d[h].src]=1;e=null}else c.eval(d[h].innerHTML)}};"click keydown keyup keypress submit load resize change select error".split(" ").forEach(function(b){m.fn[b]=function(c){return c?this.bind(b,c):this.trigger(b)}});["focus","blur"].forEach(function(b){m.fn[b]=
