...
else c(window).one("afui:init",function(){f.autoBoot()});else c(document).ready(function(){if(f.init)f.autoBoot();else c(window).one("afui:init",function(){f.autoBoot()})},!1);"AppMobi"in window||(window.AppMobi={},window.AppMobi.webRoot="");window.addEventListener("popstate",function(){if(f.useInteralRouting){var a=f.getPanelId(document.location.hash);"#"!=document.location.href.replace(document.location.origin+"/","")&&(""===a&&1===f.history.length&&(a="#"+f.firstDiv.id),""!==a&&0!==af(a).filter(".panel").length&&
...
"hidden");c(document).on("click","#header #backButton",function(b){b.preventDefault();a.goBack()});this.titleBar=c.query("#header #pageTitle").get(0);this.addContentDiv("afui_ajax","");var f=c.create("div",{id:"afui_mask",className:"ui-loader",html:"<span class='ui-icon ui-icon-loading spin'></span><h1>Loading Content</h1>"}).css({"z-index":2E4,display:"none"});document.body.appendChild(f.get(0));f=c.create("div",{id:"afui_modal"}).get(0);f.appendChild(c.create("div",{id:"modalContainer"}).get(0));
this.modalTransContainer=c.create("div",{id:"modalTransContainer"}).appendTo(f).get(0);this.viewportContainer.append(f);this.scrollingDivs.modal_container=c.query("#modalContainer").scroller({scrollBars:!0,vertical:!0,vScrollCSS:"afScrollbar",noParent:!0,lockBounce:this.lockPageBounce});this.modalWindow=f;for(var e={},f=this.viewportContainer.get(0).querySelectorAll(".panel"),b=0;b<f.length;b++){var h=f[b],g=h,n,r=h.previousSibling;h.parentNode&&"content"!=h.parentNode.id?(h.parentNode.removeChild(h),
n=h.id,g.getAttribute("selected")&&(this.firstDiv=c.query("#"+n).get(0)),this.addDivAndScroll(g),c.query("#"+n).insertAfter(r)):h.parsedContent||(h.parsedContent=1,h.parentNode.removeChild(h),n=h.id,g.getAttribute("selected")&&(this.firstDiv=c.query("#"+n).get(0)),this.addDivAndScroll(g),c.query("#"+n).insertAfter(r));h.getAttribute("data-defer")&&(e[n]=h.getAttribute("data-defer"));this.firstDiv||(this.firstDiv=c.query("#"+n).get(0));h=null}var f=null,D=!1,E=Object.keys(e).length;if(0<E){var D=!0,
