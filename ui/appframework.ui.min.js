...
error:a,complete:a,context:void 0,timeout:0,crossDomain:null};m.jsonP=function(b){if(C)return b.type="get",b.dataType=null,m.get(b);var a="jsonp_callback"+ ++T,d="",h,e=t.createElement("script");c[a]=function(h){clearTimeout(d);m(e).remove();delete c[a];b.success.call(void 0,h)};-1!==b.url.indexOf("callback=?")?e.src=b.url.replace(/=\?/,"="+a):(h=b.jsonp?b.jsonp:"callback",-1===b.url.indexOf("?")?b.url+="?"+h+"="+a:b.url+="&"+h+"="+a,e.src=b.url);b.error&&(e.onerror=function(){clearTimeout(d);b.error.call(void 0,
...
(c=[c]);m.isArray(a)||(a=[]);for(var h=0;h<c.length;h++)if(b.__events[c[h]])for(var e=b.__events[c[h]].slice(0),f=0;f<e.length;f++)m.isFunction(e[f])&&!1===e[f].apply(b,a)&&(d=!1);return d}};m.unbind=function(b,c,a){if(b.__events){m.isArray(c)||(c=[c]);for(var d=0;d<c.length;d++)if(b.__events[c[d]])for(var h=b.__events[c[d]],e=0;e<h.length;e++)if(a==p&&delete h[e],h[e]==a){h.splice(e,1);break}}};m.proxy=function(b,c,a){return function(){return a?b.apply(c,a):b.apply(c,arguments)}};var V=function(b,
c){for(var a=0;a<b.length;a++)z(b[a],c)};m.cleanUpContent=function(b,c,a){if(b){var d=b.childNodes;d&&0<d.length&&m.asap(V,{},[J.apply(d,[0]),a]);c&&E(b,a)}};var I=[],L=[],M=[];m.asap=function(b,a,d){if(!m.isFunction(b))throw"$.asap - argument is not a valid function";I.push(b);L.push(a?a:{});M.push(d?d:[]);c.postMessage("afm-asap","*")};c.addEventListener("message",function(b){b.source==c&&"afm-asap"==b.data&&(b.stopPropagation(),0<I.length&&I.shift().apply(L.shift(),M.shift()))},!0);var N={};m.parseJS=
function(b){if(b){if("string"==typeof b){var a=t.createElement("div");C?MSApp.execUnsafeLocalFunction(function(){a.innerHTML=b}):a.innerHTML=b;b=a}var d=b.getElementsByTagName("script");b=null;for(var h=0;h<d.length;h++)if(0<d[h].src.length&&!N[d[h].src]&&!C){var e=t.createElement("script");e.type=d[h].type;e.src=d[h].src;t.getElementsByTagName("head")[0].appendChild(e);N[d[h].src]=1;e=null}else c.eval(d[h].innerHTML)}};"click keydown keyup keypress submit load resize change select error".split(" ").forEach(function(b){m.fn[b]=
