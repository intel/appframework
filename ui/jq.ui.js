...
$.ui =  (function () {
   ...
   var ui = function () {
		...
		if (window.AppMobi)
			document.addEventListener("appMobi.device.ready", function () { that.hasLaunched = true; that.launch(); }, false);
		else 
		   if (document.readyState == "complete" || document.readyState == "loaded")
		      that.hasLaunched=true,that.launch();
		else 
		   document.addEventListener("DOMContentLoaded", function () { that.hasLaunched = true; that.launch(); }, false);
		
		window.addEventListener('orientationchange',function(e){window.scrollTo(0,1)}, false);
    };
    ...
    ui.prototype = {
		...
		catch(e){console.log("Error animating "+e.message+"  "+el.id);}
		},
        ...
        launch: function () {
            ...
            document.addEventListener("appMobi.device.orientation.change",function(){ that.updateOrientation()}, false);
...
//The following are functions for preventing the page to move and hiding the address bar in iOS - android has some bugs with the layouts
//We cache the initial positions because Android randomly changes the heights.
(function(){
//Hacks for address bar
	...
	fixTopBar=function fixTopBar(force){
		...
		$("#content").css("height",window.innerHeight*2+"px");
		if($.os.ios)
			window.scrollTo(0,1); 
	...
	window.addEventListener("load",function(){initJQUi();},false);

	window.addEventListener("orientationchange",function(){fixTopBar(false,window.orientation)},false);
